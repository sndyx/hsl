...
class HumanEmitter(
    ...
    override fun print(diagnostic: Diagnostic) {
        ...
        val stackTrace = diagnostic.stackTrace ?: Thread.currentThread().stackTrace.toList().drop(4)
        ...
        diagnostic.spans.getOrNull(0)?.let { (span, msg) ->
            sourceMap.files[span.fid]?.let { file ->
                ...
                stackTrace.take(5).forEach {
                    ...
                    t.println(" ${bold("-+>")} ${underline("${it.fileName}:${it.lineNumber}")} @ ${it.methodName}")
                }
    ...
    override fun start(name: String, location: String) {
        val t = Terminal()
        t.println("${(green + bold)("compiling:")} ${bold(name)} ${(italic + gray)(location)}")
    }

    override fun complete(name: String, time: Long) {
        val t = Terminal()
        t.println("${(green + bold)("complete:")} ${bold(name)} successfully compiled in ${italic("${time}ms")}")
    }
    ...
    override fun pass(name: String, time: Long) {
        val t = Terminal()
        t.println("${(white + bold)("pass:")} ${bold(name)} completed in ${italic("${time}ms")}")
    }
